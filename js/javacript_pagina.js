...
function buscar(){
   ...
   var header = document.getElementsByTagName("header")
     for(i; i < json.length; i++){
        if((json[i].tipo_coche == "Compacto" || json[i].tipo_coche == "SUV") && json[i].combustible == "Gasolina"){
            var tarjeta = document.createElement("div")
            tarjeta.setAttribute("class", "tarjeta")
            var espacioIm = document.createElement("div")
            espacioIm.setAttribute("class", "espacio-imagen")
            ...
            segundaFila.appendChild(datos)
            segundaFila.appendChild(aniadir)
...
function nombreCoche(div){
   for(var i = 0; i < div.childNodes.length; i++){
      alert(div.childNodes[i].className)
   }
}